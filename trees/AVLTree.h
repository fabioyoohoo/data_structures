//
// Created by Frederick Hall on 2019-02-13.
//
...
private:
    ...
    int find(const Comparable &c, AVLNode* n, int counter) const {
        if (n == nullptr) {
        ...
        if (c < n->value) {
            ...
            ++ counter;
            return find(c, n->leftChild, counter);
        }
        if (n->value < c) {
            // Value is greater than current node. Go to the node's right child.
            ++ counter;
            return find(c, n->rightChild, counter);
        }
    ...
    void add(const Comparable &c, AVLNode* &n) {
        ...
        // Now that we have added the node, balancing the tree is necessary.
    ...
    void remove(const Comparable &c, AVLNode* &n) {
        if (n == nullptr) {
            // We did not find the value. Cannot remove it. Do nothing.
            return;
        }
        else if (c < n->value) {
            // Value is less than current node. Go to left child.
        ...
        // If code reaches here, we fond the node. Now remove it.
...
public:
    ...
    int find(const Comparable &c) const {
        // initiate counter to record depth of node visited
        int counter = 1;
        // calls private helper function
        return find(c, root, counter);
    }
