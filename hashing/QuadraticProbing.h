//
// Created by Frederick Hall on 2019-04-13.
//
...
public:
// Constructor
    QuadraticProbing(int table_size, function<string(Hashable)> get_key) {
    ...
    vector<int> insert(Hashable item) {
        // set reads to zero
        reads = 0;
        // declare the vector to hold reads and table size
        vector<int> data;
        ...
        while (hash_table[index].condition == FULL) {
            index = index + pow(2,collisions);
            // increase collisions tracker and reads count
            ++collisions;
            ++reads;
    ...
    optional<Hashable> find(string key) {
        ...
        while (hash_table[index].condition != EMPTY) {
            ...
            // If the index is greater than 2 times the table size we know it's hopeless
            // to find the key.
            if (index > (2 * table_size)) {
                // Search failed
                return nullopt;
            }
        }
...
#endif //HASHING_QUADRATICPROBING_H
